...
const DEFAULT_COLUMNS: Column[] = [
  { id: 'orderNumber', label: 'Nr zlecenia', sortable: true, align: 'left', visible: true },
  { id: 'createdAt', label: 'Data', sortable: true, align: 'left', visible: true },
  { id: 'client', label: 'Klient', sortable: true, align: 'left', visible: true },
  { id: 'project', label: 'Projekt', sortable: true, align: 'left', visible: true },
  { id: 'system', label: 'System', sortable: true, align: 'left', visible: true },
  { id: 'totalWindows', label: 'Okna', sortable: true, align: 'center', visible: true },
  ...
  { id: 'glassDelivery', label: 'Dostawa szyb', sortable: true, align: 'left', visible: true },
  { id: 'valuePln', label: 'Wartość PLN', sortable: true, align: 'right', visible: true },
  { id: 'valueEur', label: 'Wartość EUR', sortable: true, align: 'right', visible: true },
  { id: 'invoiceNumber', label: 'Nr faktury', sortable: false, align: 'left', visible: true },
  { id: 'orderStatus', label: 'Status zamówienia', sortable: true, align: 'center', visible: true },
  { id: 'pvcDelivery', label: 'Dostawa PVC', sortable: true, align: 'left', visible: true },
  { id: 'deadline', label: 'Termin realizacji', sortable: true, align: 'left', visible: true },
  { id: 'archived', label: 'Status', sortable: true, align: 'center', visible: true },
];
...
export default function ZestawienieZlecenPage() {
  ...
  const [sortField, setSortField] = useState<ColumnId>('createdAt');
  ...
  const filteredOrders = useMemo(() => {
    ...
    return result;
  }, [allOrders, searchQuery, sortField, sortDirection]);
  ...
  const getGroupKey = (order: any, groupBy: GroupBy): string => {
    switch (groupBy) {
      case 'client':
  ...
  const getCellValue = (order: any, columnId: ColumnId): string => {
    switch (columnId) {
      case 'orderNumber':
      ...
      case 'archived':
        return order.archived ? 'Archiwum' : 'Aktywne';
      default:
        return '';
    }
  };
  ...
  const SortHeader = ({ column }: { column: Column }) => {
    ...
    return (
      ...
      >
        {column.label}
        <ArrowUpDown className={`h-3 w-3 ${sortField === column.id ? 'text-blue-600' : 'text-slate-400'}`} />
      </button>
    );
  };

  const renderCell = (order: any, column: Column) => {
    ...
    switch (column.id) {
      ...
      case 'glassDelivery':
      case 'deadline':
        const dateValue = column.id === 'createdAt'
          ? order.createdAt
          : column.id === 'pvcDelivery'
          ? order.pvcDeliveryDate
          : column.id === 'glassDelivery'
      ...
      case 'valueEur':
        ...
        return (
          <td key={column.id} className={`px-4 py-3 ${alignClass}`}>
            {value ? formatCurrency(parseFloat(value), currency) : '-'}
          </td>
        );
        ...
        return (
          <td key={column.id} className={`px-4 py-3 ${alignClass}`}>
            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
              ...
              {order.archived ? 'Archiwum' : 'Aktywne'}
            </span>
          </td>
        );
