...
export default function UstawieniaPage() {
  ...
  const {
    errors: authorMappingErrors,
    touched: authorMappingTouched,
    validate: validateAuthorMappingField,
    validateAll: validateAuthorMappingForm,
    touch: touchAuthorMappingField,
    reset: resetAuthorMappingValidation,
  } = useFormValidation({
    authorName: [
      { validate: (v: string) => !!v?.trim(), message: 'Nazwa autora jest wymagana' },
      { validate: (v: string) => v?.trim().length >= 2, message: 'Nazwa musi mieć min. 2 znaki' },
    ],
  ...
  const { data: fileWatcherStatus, refetch: refetchFileWatcherStatus } = useQuery({
    ...
    queryFn: settingsApi.getFileWatcherStatus,
  });
  ...
  const { createMutation: createAuthorMappingMutation, updateMutation: updateAuthorMappingMutation, deleteMutation: deleteAuthorMappingMutation } =
    useDocumentAuthorMappingMutations({
      onCreateSuccess: () => {
        setAuthorMappingDialog({ open: false, mode: 'add', data: null });
  ...
  const handleSaveAuthorMapping = () => {
    ...
    if (!validateAuthorMappingForm(formData)) {
      Object.keys(formData).forEach((field) => touchAuthorMappingField(field as keyof typeof formData));
      return;
    }

    if (authorMappingDialog.mode === 'add') {
      createAuthorMappingMutation.mutate(formData);
    } else if (data.id) {
      updateAuthorMappingMutation.mutate({ id: data.id, data: formData });
    }
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 p-6 overflow-auto">
        <Tabs defaultValue="general">
          ...
          <TabsContent value="author-mappings">
            <DocumentAuthorMappingsTab
              mappings={authorMappings}
              onAdd={() =>
                setAuthorMappingDialog({
                  open: true,
                  mode: 'add',
                  data: { authorName: '', userId: 0 },
                })
              }
              onEdit={(mapping) => setAuthorMappingDialog({ open: true, mode: 'edit', data: mapping })}
              onDelete={(mapping) =>
                setDeleteDialog({ open: true, type: 'author-mapping', id: mapping.id, name: mapping.authorName })
              }
            />
          </TabsContent>
      ...
      <DocumentAuthorMappingDialog
        open={authorMappingDialog.open}
        mode={authorMappingDialog.mode}
        data={authorMappingDialog.data}
        users={users}
        onOpenChange={(open) => {
          if (!open) {
            setAuthorMappingDialog({ open: false, mode: 'add', data: null });
        ...
        onDataChange={(data) => setAuthorMappingDialog((prev) => ({ ...prev, data }))}
        onSave={handleSaveAuthorMapping}
        isPending={createAuthorMappingMutation.isPending || updateAuthorMappingMutation.isPending}
        errors={authorMappingErrors}
        touched={authorMappingTouched}
        onValidateField={validateAuthorMappingField}
