/**
 * Skrypt do usuwania duplikatów z tabeli loose_glasses
 *
 * Duplikat = ten sam orderNumber + widthMm + heightMm + quantity + glassComposition
 * Zostawiamy NAJNOWSZY rekord (największe id), usuwamy starsze
 *
 * Uruchom: node scripts/cleanup-loose-glass-duplicates.cjs
 */
...
async function main() {
  console.log('========================================');
  console.log('CZYSZCZENIE DUPLIKATÓW LOOSE_GLASSES');
  console.log('========================================\n');

  // 1. Pobierz wszystkie rekordy
  const allGlasses = await prisma.looseGlass.findMany({
  ...
  // 2. Znajdź duplikaty
  // Klucz: orderNumber|widthMm|heightMm|quantity|glassComposition
  const seen = new Map();
  const duplicateIds = [];
  ...
  for (const glass of allGlasses) {
    ...
    const key = `${glass.orderNumber}|${glass.widthMm}|${glass.heightMm}|${glass.quantity}|${composition}`;

    if (seen.has(key)) {
      // To jest duplikat (starszy rekord, bo sortujemy desc po id)
      duplicateIds.push(glass.id);

      // Pokaż co usuwamy (dla debug)
  ...
  for (const [name, count] of Object.entries(byCustomer).sort((a, b) => b[1] - a[1])) {
  ...
  if (!autoConfirm) {
    ...
    console.log('   Przykład: node scripts/cleanup-loose-glass-duplicates.cjs --confirm\n');
  ...
  const result = await prisma.looseGlass.deleteMany({
    where: {
      id: { in: duplicateIds }
    }
  });
  ...
  const afterCount = await prisma.looseGlass.count();
