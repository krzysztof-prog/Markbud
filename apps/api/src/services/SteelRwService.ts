...
export class SteelRwService {
  ...
  /**
   * Cofnij RW dla zlecenia - przywróć stany magazynowe stali
  ...
  async reverseRwForOrder(orderId: number, userId?: number): Promise<RwProcessResult> {
    ...
    // Znajdź zlecenie z completed steel requirements
    const order = await this.prisma.order.findUnique({
      where: { id: orderId },
      select: {
        id: true,
        ...
        steelRequirements: {
          ...
          include: {
            steel: { select: { id: true, name: true, articleNumber: true } },
          },
        },
      },
    });
    ...
    if (!order) {
      logger.warn('Order not found for Steel RW reversal', { orderId });
      return result;
    }
    ...
    if (order.steelRequirements.length === 0) {
      logger.info('No completed steel requirements to reverse', { orderId });
      return result;
    }

    logger.info('Reversing Steel RW for order', {
      orderId,
      orderNumber: order.orderNumber,
      requirementsCount: order.steelRequirements.length,
    });
    ...
    await this.prisma.$transaction(async (tx) => {
      for (const requirement of order.steelRequirements) {
        try {
          ...
          const newQty = previousQty + reverseQty;
          ...
          await tx.steelHistory.create({
            data: {
              ...
              previousStock: previousQty,
              currentStock: newQty,
              changeType: 'rw_reverse',
              notes: `Cofnięcie RW - Zlecenie ${order.orderNumber}, zmiana: +${reverseQty}`,
              recordedById: userId ?? null,
            },
          });
          ...
          logger.debug('Reversed Steel RW', {
            steelId: requirement.steelId,
            orderId,
            previousQty,
            reverseQty,
    ...
    logger.info('Steel RW reversal completed', {
      orderId,
      orderNumber: order.orderNumber,
      processed: result.processed,
      skipped: result.skipped,
    ...
    if (result.processed > 0) {
      ...
      emitSteelStockUpdated({ source: 'rw_reverse', orderId });
    }
