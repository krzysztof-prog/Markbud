...
import { Package, AlertTriangle, Pencil, Check, X, ChevronDown, ChevronRight, Plus, Trash2, ArrowLeft, Warehouse, FileText, ClipboardCheck } from 'lucide-react';
        ...
        <CardContent className="p-0">
          ...
          <table className="w-full text-sm min-w-[800px]">
            <thead className="bg-slate-50 border-b sticky top-0 z-20">
              <tr>
                <th className="px-4 py-3 text-left font-medium text-slate-700 sticky left-0 bg-slate-50 z-30">
                  Zlecenie
                </th>
              ...
              <tr className="bg-slate-100 border-b text-xs">
                <th className="px-4 py-2 text-left text-slate-500 sticky left-0 bg-slate-100 z-30"></th>
                {profiles.map((profile: OrderTableData['profiles'][0]) => (
                  <>
                    <th key={`${profile.id}-bele`} className="px-2 py-2 text-center text-slate-700 font-semibold border-l-4 border-l-blue-400 bg-blue-100">
                      bele
                    </th>
                    <th key={`${profile.id}-m`} className="px-2 py-2 text-center text-slate-500 border-l">
                      m
                    </th>
                  </>
                ))}
          ...
          <tbody>
            ...
            <tr className="bg-blue-50 font-semibold sticky top-[74px] z-10">
              <td className="px-4 py-3 sticky left-0 bg-blue-50 z-20">SUMA</td>
              {profiles.map((profile: OrderTableData['profiles'][0]) => {
                ...
                return (
                  <>
                    <td
                      ...
                      className="px-2 py-3 text-center border-l-4 border-l-blue-400 bg-blue-200 font-bold"
                    >
                      {total.beams}
                    </td>
                    <td key={`total-${profile.id}-m`} className="px-2 py-3 text-center border-l bg-blue-50">
            ...
            {orders.map((order: OrderTableData['orders'][0], index: number) => {
              ...
              <tr key={order.orderId} className={`border-b hover:bg-slate-100 ${rowBg}`}>
                <td className={`px-4 py-3 font-mono font-medium sticky left-0 z-10 ${rowBg}`}>
                  <button
                  ...
                  return (
                    <>
                      <td
                        ...
                        className="px-2 py-3 text-center border-l-4 border-l-blue-400 bg-blue-50 font-medium"
                      >
                        {req.beams || '-'}
                      </td>
                      <td key={`${order.orderId}-${profile.id}-m`} className="px-2 py-3 text-center border-l">
                        {req.meters || '-'}
                      </td>
                    </>
                  );
